...
export async function postExt(url) {
    try {
        const response = await fetch(url, setHeaders('POST'));
        if (response.status === 200) {
            return Promise.resolve();
        } else {
            Promise.reject('Error in post EXT');
        }
    } catch (e) {
        return Promise.reject(e);
    }
...
export async function postMultiPart(url, obj) {
    ...
    try {
        ...
        return Promise.reject({ error: 'Error from posting multi part' });
    }
}
